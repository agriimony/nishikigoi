...
class Leaf {
  ...
  display() {
    ...
    for (let i = 0; i < this.lines; i++) {
      ...
      push();
      rotate(a*i);
      line(0.15 * this.size / 2, 0, this.size / 4 + this.size / 5 * noise(time+i* 10), 0);
      pop();
    }
...
class Fish {
    ...
    display_fin(fin_type) {
      ...
      if (fin_type == "classic") { // use classic fin shape
        ...
        for (let i = 0; i < 50; i+=5){
          ...
          let x2 = 0.025 * this.fl + i * noise(r + 0) * M;
          let x3 = 0.2 * this.fl + i * noise(r + 15) * M;
          let x4 = 0.15 * this.fl + 1/(i+1) * noise(r + 30) * M;
          let y1 = 0 + i * M;
          let y2 = -0.05 * this.fl + i * noise(r + 40) * M;
          let y3 = 0.05 * this.fl + i * noise(r + 50) * M;
          let y4 = 0.05 * this.fl + 1/(i+1) * noise(r + 60) * M;
    ...
    display_top_fin(fin_type) {
      ...
      if (fin_type == "classic") {
        ...
        for (let i = 0; i < 50; i+=5){
          let x1 = startx;
          let x2 = startx + 10 * cos(this.turning) * M - i * noise(r + 0) * M;
          let x3 = endx - cos(this.turning) * 4 * i * noise(r + 15) * M;
          let x4 = endx;
          let y1 = starty;
          let y2 = starty - i * noise(r + 40) * M;
          let y3 = endy - sin(this.turning) * 2 * i * noise(r + 50) * M;
          let y4 = endy;
    ...
    display_tail() {
      ...
      if (this.fin_type == "classic") {
        ...
        this.fincol.setAlpha(100);
        stroke(this.fincol);
        rotate(sin(map(noise(time/4 + this.speed), 0,1,-0.2,0.8)));
        let s = time/2 + this.speed;
        smooth();
        beginShape();
        curveVertex(0,0);
        curveVertex(0,0);
        curveVertex(0,this.fl*0.05);
        curveVertex(this.fl * 0.375 * noise(s), this.fl * 0.15 * noise(s));
        curveVertex(this.fl * 0.35, this.fl * 0.025);
        curveVertex(this.fl * 0.175 * noise(s), 0);
        curveVertex(this.fl * 0.2 * noise(s) * 2, -this.fl * 0.05 * noise(s));
        curveVertex(this.fl * 0.25, -this.fl * 0.15 * noise(s)*2);
        curveVertex(0, -this.fl*0.025);
        curveVertex(0,0);
        curveVertex(0,0);
        endShape(CLOSE);
        pop();
